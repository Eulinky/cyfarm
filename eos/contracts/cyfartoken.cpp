#include "cyfartoken.hpp"
#include <math.h>

ACTION cyfartoken::setconfig(const symbol_code& sym, const string& version) {
...
ACTION cyfartoken::create(const name& issuer,
...
ACTION cyfartoken::issue(const name& to,
                     const name& category,
                     const name& token_name,
...
ACTION cyfartoken::burnnft(const name& owner,
...
ACTION cyfartoken::burnft(const name& owner,
...
ACTION cyfartoken::transfernft(const name& from,
                           const name& to,
                           const vector<uint64_t>& dgood_ids,
                           const string& memo ) {
...
ACTION cyfartoken::transferft(const name& from,
                          const name& to,
                          const name& category,
                          const name& token_name,
                          const asset& quantity,
...
ACTION cyfartoken::listsalenft(const name& seller,
                           const vector<uint64_t>& dgood_ids,
...
ACTION cyfartoken::logcall(const uint64_t& dgood_id) {
    require_auth( get_self() );
}

// method to logsuccessful sale
ACTION cyfartoken::logsale(const vector<uint64_t>& dgood_ids, const name& seller, const name& buyer, const name& receiver) {
    require_auth( get_self() );
}

ACTION cyfartoken::freezemaxsup(const name& category, const name& token_name) {
    require_auth( get_self() );
...
// Private
map<name, asset> cyfartoken::_calcfees(vector<uint64_t> dgood_ids, asset ask_amount, name seller) {
...
// Private
void cyfartoken::_changeowner(const name& from, const name& to, const vector<uint64_t>& dgood_ids, const string& memo, const bool& istransfer) {
    check (dgood_ids.size() <= 20, "max batch size of 20");
    // loop through vector of dgood_ids, check token exists
    dgood_index dgood_table( get_self(), get_self().value );
    lock_index lock_table( get_self(), get_self().value );
...
// Private
void cyfartoken::_checkasset(const asset& amount, const bool& fungible) {
...
// Private
void cyfartoken::_mint(const name& to,
                   const name& issuer,
                   const name& category,
                   const name& token_name,
...
uint64_t cyfartoken::_nextdgoodid() {
...
// Private
void cyfartoken::_add_balance(const name& owner, const name& ram_payer, const name& category, const name& token_name,
...
// Private
void cyfartoken::_sub_balance(const name& owner, const uint64_t& category_name_id, const asset& quantity) {
